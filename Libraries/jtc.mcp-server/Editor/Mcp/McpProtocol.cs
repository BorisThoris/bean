...
public class JsonRpcResponse
{
	...
	public static JsonRpcResponse Ok( JsonElement id, object result ) =>
		new() { Id = ToStableId( id ), Result = result };

	public static JsonRpcResponse Fail( JsonElement id, int code, string message ) =>
		new() { Id = ToStableId( id ), Error = new JsonRpcError { Code = code, Message = message } };

	private static object ToStableId( JsonElement id )
	{
		return id.ValueKind switch
		{
			JsonValueKind.String => id.GetString(),
			JsonValueKind.Number when id.TryGetInt64( out var longValue ) => longValue,
			JsonValueKind.Number when id.TryGetDouble( out var doubleValue ) => doubleValue,
