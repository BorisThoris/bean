...
- Attempts to load the Construct map with `SceneMap.CreateAsync` using `facepunch.construct`.
- Builds the tapper arena at runtime inside the Construct environment, with a generated fallback if Construct is unavailable.
...
- Keeps a physical scoreboard, station status text, heat/progress bars, button punch feedback, winner glow, and result summaries.
